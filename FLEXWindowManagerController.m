...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIWindow *window = [self windowAtIndexPath:indexPath];
    cell.textLabel.text = [self titleForWindow:window];
    
    // 设置副标题
    if (window == [UIApplication sharedApplication].keyWindow) {
        cell.detailTextLabel.text = self.keyWindowSubtitle;
    } else if (indexPath.row < self.windowSubtitles.count) {
        cell.detailTextLabel.text = self.windowSubtitles[indexPath.row];
    } else {
...
#pragma mark - 辅助方法
// 根据 IndexPath 获取对应的窗口
- (UIWindow *)windowAtIndexPath:(NSIndexPath *)indexPath {
    NSArray *windows = [UIApplication sharedApplication].windows;
    if (indexPath.row < windows.count) {
        return windows[indexPath.row];
    }
    return nil;
}

// 获取窗口的标题
- (NSString *)titleForWindow:(UIWindow *)window {
    if (window == [UIApplication sharedApplication].keyWindow) {
        return [NSString stringWithFormat:@"Key Window (%@)", NSStringFromClass([window class])];
    }
