...
// 添加实例变量声明
@property (nonatomic, assign) BOOL subclassesAreSorted; // 替换 _subclassesAreSorted 
@property (nonatomic, assign) BOOL shouldSortSubclasses; // 替换 _shouldSortSubclasses
@end
...
- (BOOL)isSubclassOfClass:(Class)parentClass {
    Class klass = NSClassFromString(self.classObjectName);
    if (!klass) return NO;
    
    Class superClass = class_getSuperclass(klass);
    while (superClass) {
        if (superClass == parentClass) return YES;
...
- (NSMutableSet *)getProtocolsAdoptedByClass {
    return [self protocolsNames];
}

- (NSArray *)sortedSubclassesStubs {
    return [self.subclassesStubs sortedArrayUsingSelector:@selector(compare:)];
}
...
- (id)classMethodNameAtIndex:(int)index {
    Class klass = objc_getMetaClass([self.classObjectName UTF8String]);
    if (!klass) return nil;
    
    unsigned int count;
    Method *methods = class_copyMethodList(klass, &count);
    if (!methods || index >= count) {
        free(methods);
        return nil;
    }
...
- (NSArray *)getClassMethodNames:(BOOL)includeParents {
    ...
    do {
        ...
        // 继续遍历父类
        klass = class_getSuperclass(klass);
    } while (klass && class_isMetaClass(klass));
