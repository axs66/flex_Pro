...
- (void)registerGlobalEntries {
    [self registerGlobalEntryWithName:@"🔍  系统浏览器" viewControllerFutureBlock:^UIViewController *{
        return [FLEXFileBrowserController new];
    }];
    
    [self registerGlobalEntryWithName:@"📱  系统信息" viewControllerFutureBlock:^UIViewController *{
        return [FLEXTableViewController new];
    }];
    
    // 添加 Dobby Hook 管理器选项
    [self registerGlobalEntryWithName:@"🪝  Dobby Hook 管理器" viewControllerFutureBlock:^UIViewController *{
        return [[FLEXTableViewController alloc] init]; // 或者使用其他适当的视图控制器
    }];
}
