...
+ (UIViewController *)instancesOfClassWithName:(NSString *)className retained:(BOOL)retain {
    // 使用现有的 instancesOfClass 方法获取实例，然后包装为 FLEXObjectRef
    Class cls = NSClassFromString(className);
    NSArray *instances = [FLEXHeapEnumerator instancesOfClass:cls];
...
+ (instancetype)objectsWithReferencesToObject:(id)object retained:(BOOL)retain {
    // 使用 enumerateLiveObjectsUsingBlock 方法查找引用
    NSMutableArray *referencingObjects = [NSMutableArray array];
    [FLEXHeapEnumerator enumerateLiveObjectsUsingBlock:^(__unsafe_unretained id obj, __unsafe_unretained Class actualClass) {
        // 检查对象是否引用了目标对象
        // 注意：这是一个简化实现，实际查找引用需要更复杂的逻辑
        if ([self isObject:obj referencingObject:object]) {
            [referencingObjects addObject:obj];
        }
    }];
    
    NSArray<FLEXObjectRef *> *instances = [FLEXObjectRef referencingAll:referencingObjects retained:retain];
...
// 添加帮助方法检查一个对象是否引用了另一个对象
+ (BOOL)isObject:(id)sourceObject referencingObject:(id)targetObject {
    // 这里需要实现检查逻辑，简单实现可能会有漏洞
    // 此处仅为示例代码
    if (!sourceObject || !targetObject) return NO;
    
    // 检查基本对象关系
    if (sourceObject == targetObject) return NO; // 自引用不算
    
    // 检查常见集合类型
    if ([sourceObject isKindOfClass:[NSArray class]]) {
        return [(NSArray *)sourceObject containsObject:targetObject];
    }
    
    if ([sourceObject isKindOfClass:[NSDictionary class]]) {
        NSDictionary *dict = (NSDictionary *)sourceObject;
        return [dict.allKeys containsObject:targetObject] || [dict.allValues containsObject:targetObject];
    }
