...
@property (nonatomic, copy) FLEXColorPickerCallback colorPickerCallback; // 新增回调属性
@end
...
- (void)startColorPicker {
    [self startColorPicker:NULL]; // 使用 NULL 而不是 nil
}

- (void)startColorPicker:(FLEXColorPickerCallback)callback {
    if (self.isColorPickerActive) return;
    
    self.isColorPickerActive = YES;
    self.colorPickerCallback = callback; // callback 可以为 nil
...
- (void)handleColorPickerTap:(UITapGestureRecognizer *)gesture {
    ...
    // 处理回调 - 增加空值检查
    if (self.colorPickerCallback) {
        self.colorPickerCallback(color);
    }
}
