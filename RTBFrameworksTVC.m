...
- (IBAction)loadAllFrameworks:(id)sender {
    ...
    typeof(self) blockSelf = self;  // 使用强引用而非弱引用
    
    [self presentViewController:alertController animated:YES completion:^{
        CGFloat margin = 8.0;
        CGFloat progressHeight = 2;
        blockSelf.progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(margin, alertController.view.frame.size.height - progressHeight, alertController.view.frame.size.width - margin * 2.0 , progressHeight)];
        blockSelf.progressView.progress = 0.0;
        [alertController.view addSubview:blockSelf.progressView];
    }];
