...
- (void)addToolbarItem:(id)toolbarItem {
    // 获取当前工具栏项
    NSMutableArray *items = [self.explorerToolbar.toolbarItems mutableCopy];
    
    // 在关闭按钮之前插入新项目
    // 确保数组有足够的项目
    if (items.count >= 2) {
        [items insertObject:toolbarItem atIndex:items.count - 1];
        self.explorerToolbar.toolbarItems = items;
    } else {
        // 如果工具栏为空或只有一项，简单地添加到末尾
        [items addObject:toolbarItem];
        self.explorerToolbar.toolbarItems = items;
    }
    
    // 更新工具栏布局
    [self.explorerToolbar setNeedsLayout];
}
